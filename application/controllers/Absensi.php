...
function export_pdf_absen_admin(){
	...
	function rekapan_absen_ketua_kelas(){
        ...
        $data['kehadiran'] = $this->db->query("SELECT * FROM absen_kelas inner join kelas on absen_kelas.id_ketua_kelas = kelas.id WHERE (id_ketua_kelas = '$id_ketua_kelas' AND MONTH(tanggal) = '$bulan' AND YEAR(tanggal) = '$tahun')")->result();
