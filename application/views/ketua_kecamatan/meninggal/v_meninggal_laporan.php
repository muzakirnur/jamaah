  ...
  <div class="card">
    ...
    <div class="card-body">
      ...
      <div class="row">
        <div class="col-md-4">
          ...
          <select name="id_gampong" id="gampong" class="form-control">
            <option value="">- Nama Gampong -</option>
            <?php
					foreach($gampong as $data){ // Lakukan looping pada variabel siswa dari controller
						echo "<option value='".$data->id."'>".$data->ketua_gampong."</option>";
					}
					?>
          </select>
        </div>
      ...
      if(isset($_GET['tanggal_mulai']) && isset($_GET['tanggal_sampai'])&& isset($_GET['id_gampong'])){
        ...
        ?>
        <a href="<?php echo base_url().'meninggal/export_pdf_meninggal_ketua_kecamatan/?tanggal_mulai='.$mulai.'&tanggal_sampai='.$sampai.'&gampong='.$gampong?>" class='btn btn-sm btn-warning pull-right'><i class="fa fa-print"></i> Print Data</a>
        <?php
      }
      ?>
      <br/>
